...
def load_destinations() -> List[Dict]:
    ...
    # Additional to reach 40+ (already >40, but add a few more)
    destinations.extend([
...
def filter_and_rank(database: List[Dict], user_config: Dict, budget_total: float) -> pd.DataFrame:
    ...
    for d in database:
        ...
        if total_cost <= budget_total:
            ...
            # budget_fit_score (0-10). Since total_cost <= budget_total, score decreases as cost approaches budget
...
# Months from current month for next 12 months
month_options = get_month_options()
default_months = month_options[:3]  # next 3 months
    ...
    for j, col in enumerate(cols):
        ...
        if idx < len(top_results):
            ...
            with col:
                ...
                # Cost breakdown in three small metrics
                with_flight = user_config["with_flight"]
                board_type_short = board_type
                # Recompute components for breakdown display
...
with tab1:
    # Bar chart: Top 10 by total cost
    ...
    # Scatter plot: Value Score vs Total Cost, bubble size = safety bonus
...
with tab3:
    # Heatmap: Destinations x Months (overlap with selected months)
    all_months = ["January","February","March","April","May","June","July","August","September","October","November","December"]
    # Create matrix: for each destination, 1 if month in best_months else 0
    heatmap_data = []
    ...
    fig_heat.update_layout(title="Destination Best Months Heatmap", xaxis_title="Month", yaxis_title="Destination",
                           height=600)
